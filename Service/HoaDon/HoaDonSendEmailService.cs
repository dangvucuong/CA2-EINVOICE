    ...
    public class HoaDonSendEmailService : BaseService, IHoaDonSendEmailService
    {
        ...
        public async Task<FunctionResult<bool>> SendEmailHoaDonAsync(List<int> hoaDonIds, bool isCheckSendBienBan = false)
        {
            ...
            foreach (var hoaDon in hoaDons)
            {
                ...
                // task.Add(_serviceWrapper.Core.Email.SendEmailAsync(new Model.Request.Email.SendEmailRequest()
                // {
                //     Body = body,
                //     // EmailAddress = new List<string>() { hoaDon.nguoi_mua_email },
                //     EmailAddress = hoaDon.nguoi_mua_email.Split(";").Select(x => x.Trim()).ToList(),
                //     isHtml = true,
                //     SendByUser = user != null ? user.full_name : "",
                //     Subject = subject,
                // }));
            }
