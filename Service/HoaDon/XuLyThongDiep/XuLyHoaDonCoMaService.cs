...
{
    public class XuLyHoaDonCoMaService : BaseService, IXuLyThongDiepService
    {
        private IHoaDonService _hoaDonService;
        private IHoaDonLogService _hoaDonLogService;
        public XuLyHoaDonCoMaService(IServiceProvider serviceProvider) : base(serviceProvider)
        {
            _hoaDonService = _serviceWrapper.HoaDon.HoaDon;
            _hoaDonLogService = _serviceWrapper.HoaDon.HoaDonLog;
        }

        public async Task<FunctionResult<HoaDonPhatHanhRespone>> XuLyThongDiepAsync(hoa_don hoaDon, Model.Respone.Xml.KetQuaThongDiepRespone thongDiepRespone, string xmlKetQua)
        {
            var isHopLe = false;
            if (thongDiepRespone.TTChung.MLTDiep == "204")
            {
                ...
                else
                {
                    ...
                    var match = Regex.Match(xmlKetQua, pattern, RegexOptions.Singleline);
                    if (match.Success)
                    {
