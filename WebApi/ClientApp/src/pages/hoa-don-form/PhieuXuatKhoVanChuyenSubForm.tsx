...
interface IPhieuXuatKhoVanChuyenSubFormProps {
  register: any;
  errors: any;
}
const PhieuXuatKhoVanChuyenSubForm = ({
  register,
  errors,
}: IPhieuXuatKhoVanChuyenSubFormProps) => {
  return (
    <Box
      display={"grid"}
      sx={{
    ...
    >
      ...
      >
        <FormControl>
          <FormControl.Label>
            <Text text="Lệnh điều động nội bộ" />
          </FormControl.Label>
          <TextInput
            register={register}
            name="xuat_kho_vc_lenh_dieu_dong_noi_bo"
            block
            required
            validateMessage="Vui lòng điền Lệnh điều động nội bộ"
            errors={errors}
          />
        </FormControl>
...
export default PhieuXuatKhoVanChuyenSubForm;
