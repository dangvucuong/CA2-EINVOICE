...
const HoaDonGocInfoModal = (props: IHoaDonGocInfoModalProps) => {
  ...
  const hinh_thuc_id = new URLSearchParams(history.location?.search).get(
    "hinh_thuc_id"
  );
  // Nếu hinh_thuc_id = 3 thì thêm filter với hoa_don_hinh_thuc_id = 3, hinh_thuc_id=2 thì thêm filter với hoa_don_hinh_thuc_id = 2, nếu không thì để trống {}
  const addFilter =
    hinh_thuc_id === "3"
      ? { hoa_don_hinh_thuc_id: 3 }
      : hinh_thuc_id === "2"
      ? { hoa_don_hinh_thuc_id: 2 }
      : {};

  const [hoaDonGocId, setHoaDonGocId] = useState(props.value?.hoaDonGocId ?? 0);
  const [
    hoa_don_dang_ky_phat_hanh_mau_so_goc,
    setHoa_don_dang_ky_phat_hanh_mau_so_goc,
  ] = useState(props.value?.hoa_don_dang_ky_phat_hanh_mau_so_goc ?? "");
  const [
    hoa_don_dang_ky_phat_hanh_ky_hieu_goc,
    setHoa_don_dang_ky_phat_hanh_ky_hieu_goc,
  ] = useState(props.value?.hoa_don_dang_ky_phat_hanh_ky_hieu_goc ?? "");
  const [ma_so_hoa_don_goc, setMa_so_hoa_don_goc] = useState(
    props.value?.ma_so_hoa_don_goc ?? ""
  );
  const [ngay_hoa_don_goc, setNgay_hoa_don_goc] = useState(
    props.value?.ngay_hoa_don_goc
      ? moment(props.value?.ngay_hoa_don_goc).format("YYYY-MM-DD")
      : ""
  );
  const [hoa_don_nghi_dinh_id_goc, setHoa_don_nghi_dinh_id_goc] = useState(
    props.value?.hoa_don_nghi_dinh_id_goc ?? 123
  );
  ...
  return (
    ...
    >
      <Box>
        <Box>
          Đối với hóa đơn <b>Đã có</b> trên hệ thống <b>Ca2 E-invoice</b>, Anh/
          Chị có thể tìm kiếm từ Hóa đơn đã tạo.
        </Box>
        <Box>
          Anh/ Chị cũng có thể điền thủ công các thông tin của hóa đơn gốc.
        </Box>
        ...
        >
          <TabNav aria-label="Main">
            <TabNav.Link
              selected={selectedTab === "ca2" ? true : undefined}
              sx={{
                cursor: "pointer",
              }}
              onClick={() => {
            ...
            >
              Hóa đơn trên Ca2 E-invoice
            </TabNav.Link>
            <TabNav.Link
              selected={selectedTab === "other" ? true : undefined}
              sx={{
                cursor: "pointer",
              }}
              onClick={() => {
            ...
            <HoaDonSelect
              ...
              onSelected={(ids, hoaDons) => {
                ...
                if (hoaDons.length === 1 && ids.length === 1) {
                  ...
                  setNgay_hoa_don_goc(
                    hoaDon?.ngay_hoa_don
                      ? moment(hoaDon?.ngay_hoa_don).format("YYYY-MM-DD")
                      : ""
                  );
                }
              }}
        ...
        {selectedTab === "other" && (
          <>
            ...
            >
              <FormControl>
                <FormControl.Label>Ký hiệu mẫu số</FormControl.Label>
                <TextInput
                  width={100}
                  value={hoa_don_dang_ky_phat_hanh_mau_so_goc}
                  onChange={(e) => {
              ...
              <FormControl>
                <FormControl.Label>Ký hiệu hóa đơn</FormControl.Label>
                <TextInput
                  value={hoa_don_dang_ky_phat_hanh_ky_hieu_goc}
                  onChange={(e) => {
