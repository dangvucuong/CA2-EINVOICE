...
const HangHoaEditFormModal = () => {
  ...
  const { user } = useAuth();
  const { hangHoaEditing, status, filter } = useAppSelector(
    (x) => x.category.hangHoaReducer
  );
  const {
    register,
    handleSubmit,
  ...
  const onSubmit = async (data: any) => {
    dispatch(
      rootAction.category.hangHoaAction.saveStart({
        ...data,
      })
    );
  };
  return (
    <Modal
      title={(hangHoaEditing?.id ?? 0) === 0 ? "Thêm mới" : "Cập nhật"}
      onClose={() => {
          ...
          <Box sx={{ display: "flex", gap: 2 }}>
            ...
            <FormControl>
              ...
              <TextInput
                ...
                // required
                width={150}
                validateMessage="Vui lòng điền Đơn giá"
                errors={errors}
              />
            </FormControl>
          </Box>
