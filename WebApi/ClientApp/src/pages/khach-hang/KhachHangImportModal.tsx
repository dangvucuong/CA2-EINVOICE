...
interface IKhachHangImportModalProps {
  onClose: () => void;
  onSuccess: () => void;
}
...
const KhachHangImportModal = (props: IKhachHangImportModalProps) => {
  ...
  const handleSubmit = async () => {
    if (isDataVilid !== "success") {
      ...
      if (uploadedFile) {
        setIsSaving(true);
        const res = await khachHangApi.importData({
          ...uploadedFile,
        });
        setIsSaving(false);
        if (res.is_success) {
...
const ValidateDataResult = (props: IValidateDataResultProps) => {
  ...
  const handleRead = async () => {
    setIsLoading(true);
    const res = await khachHangApi.validImport({
      ...props.file,
    });
    setIsLoading(false);
    if (res.is_success) {
...
export default KhachHangImportModal;
