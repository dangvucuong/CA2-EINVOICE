...
const UserPage = () => {
  ...
  const isCanNotView = useMemo(() => {
    return (
      !checkAccesiableTo(USER_API_ENDPOINT, "GET") &&
      !checkAccesiableTo(USER_API_ENDPOINT + "/don-vi/{donvi_ma_dv}", "GET")
    );
  }, []);
  ...
  const isCanNotDelete = useMemo(() => {
    return !checkAccesiableTo(USER_API_ENDPOINT + "/{id}", "DELETE");
  }, []);
  useEffect(() => {
    dispatch(
      rootAction.user.userAction.loadStart({
        ...filter,
      }),
    );
    // if (isCanViewAll) {
  ...
  return (
    <div>
        ...
        <DataTableRemotePaging
          ...
          searchConfig={{
            ...
            onValueChanged: (key: string) => {
              dispatch(
                rootAction.user.userAction.changeFilter({
                  ...filter,
                  page_index: 0,
                  search_key: key,
                }),
                    ...
                    {!isCanNotEdit && (
                      ...
                      >
                        <IconButton
                          ...
                          onClick={() => {
                            dispatch(
                              rootAction.user.userAction.showEditModal(row),
                            );
                          }}
                        />
                      </Box>
                    ...
                    {!isCanNotDelete && (
                      ...
                      >
                        <IconButton
                          ...
                          onClick={() => {
                            dispatch(
                              rootAction.user.userAction.showDeleteConfirm(row),
                            );
                          }}
                        />
                      </Box>
      ...
      {isShowEditModal &&
        (userEditingId === 0 || userEditingForm?.id === userEditingId) && (
          <UserEditFormModal />
        )}

      {isShowDeleteConfirm && userEditingForm && (
        <ConfirmModal
          onCancel={() => {
            dispatch(rootAction.user.userAction.closeDeleteConfirm());
          }}
          type="danger"
          ...
          onConfirm={() => {
            dispatch(
              rootAction.user.userAction.deleteStart(userEditingForm?.id ?? 0),
            );
          }}
        />
      )}
