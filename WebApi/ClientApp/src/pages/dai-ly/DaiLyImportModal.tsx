...
interface IDaiLyImportModalProps {
  onClose: () => void;
  onSuccess: () => void;
}
...
const DaiLyImportModal = (props: IDaiLyImportModalProps) => {
  ...
  const handleSubmit = async () => {
    if (isDataVilid !== "success") {
      ...
      if (uploadedFile) {
        setIsSaving(true);
        const res = await daiLyApi.importData({
          ...uploadedFile,
        });
        setIsSaving(false);
        if (res.is_success) {
...
const ValidateDataResult = (props: IValidateDataResultProps) => {
  ...
  const handleRead = async () => {
    setIsLoading(true);
    const res = await daiLyApi.validImport({
      ...props.file,
    });
    setIsLoading(false);
    if (res.is_success) {
...
export default DaiLyImportModal;
